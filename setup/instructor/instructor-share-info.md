...
> **ドキュメント編集時のライブリロード**
>
> Docker版MkDocs（ポート8001）は`--watch-poll`オプションによりポーリングベースのファイル監視を使用しています。
> これにより、Dockerコンテナ環境でもファイル変更が自動的に検知され、ブラウザに反映されます。
>
> ただし、ポーリング間隔（1-2秒）があるため、即座の反映が必要な場合は以下の手順で**ローカル版MkDocs**を併用できます：
>
> ```bash
> # プロジェクトルートで実行
...
> ```
>
> - **編集作業用**: http://localhost:8000（ローカル版、即座に反映）
> - **受講者共有用**: http://localhost:8001（Docker版、ネットワーク共有）
>
> 通常はDocker版（8001）のみで十分ですが、頻繁に編集する場合はローカル版の併用を推奨します。
