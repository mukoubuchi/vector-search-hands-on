...
> **ドキュメント編集時の注意事項**
>
> **macOS Docker Desktopの制限により、Docker版MkDocs（ポート8001）ではファイル変更の自動検知が動作しません。**
>
> ### コンテナ版 vs 開発版の比較
>
> | 項目 | コンテナ版（ポート8001） | 開発版（ポート8000） |
> |------|------------------------|---------------------|
> | **起動方法** | `./start-all.sh`で自動起動 | `python -m mkdocs serve`で手動起動 |
> | **用途** | 受講者への共有、安定配信 | ドキュメント編集作業 |
> | **自動リロード** | ❌ 不可（macOS Docker Desktop制限） | ✅ 可能 |
> | **変更反映** | コンテナ再起動またはブラウザ手動リロード | 即座に反映 |
> | **ネットワーク共有** | ✅ 可能（講師のIP:8001） | ✅ 可能（講師のIP:8000） |
...
>    ./start-all.sh
>    ```
>    - アクセス: http://localhost:8001 または http://講師のIP:8001
...
#### 同時起動のメリット
- ドキュメント編集中も受講者は8001にアクセス可能
- 編集作業（8000）と配信（8001）を並行できる
- 編集内容を8000で確認しながら、受講者は8001で安定版を閲覧

#### 同時起動のデメリット
- **リソース消費**: 同じコンテンツを2つのプロセスで配信（メモリ・CPU使用）
- **混乱のリスク**: どちらのポートを使っているか分からなくなる可能性
- **メンテナンス性**: 2つのサーバーを管理する必要がある
...
```
- 受講者は http://講師のIP:8001 にアクセス
- ドキュメント編集は行わない（安定配信に専念）

**ドキュメント編集が必要な場合:**
```bash
# プロジェクトルート（mkdocs.ymlがある場所）に移動
cd /path/to/vector-search-handson
...
# バックグラウンド実行時
cd setup/instructor && ./stop-all.sh  # ポート8000を自動検出して停止
...
- 受講者は引き続き http://講師のIP:8001 を使用
- 編集完了後、コンテナを再起動すれば8001にも反映
...
- **コンテナ版（8001）**: ファイル変更の自動検知が動作しない（macOS Docker Desktop制限）
