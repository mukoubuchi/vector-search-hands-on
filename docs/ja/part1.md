...
??? note "任意: API の生のレスポンスを見る"
    検索画面が表示している JSON を見るには、Swagger UI（**`http://localhost:8002/docs`**）で **`/search`** を開き、「Try it out」をクリックして以下のリクエストボディを入力し、「Execute」をクリックします。Swagger UI はファイルをインターネットから読み込みますが、検索画面はオフラインでも動きます。
