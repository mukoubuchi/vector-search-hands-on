...
このパートでは、Part 2 で追加した機能が正しく動作するか確認し、IBM Bob のコードレビュー機能を使います。
...
## ステップ 2: IBM Bob のコードレビュー機能を使う（5 分）
